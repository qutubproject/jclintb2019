...
// Medications setup
  ...
  use "$DATA_DIR/SP Medications.dta" , clear
...
// Full TB Classic interactions data
  ...
  // South Africa
  use "$DATA_DIR/SouthAfrica.dta" , clear
    gen study = " South Africa"
    gen facility_type = "PHC"

    egen checklist = rowmean(ask*)

    // RECONSTRUCTED VARIABLE. READ BEFORE USING.
    //
    // The row-level "any medicine given" indicator could not be shared for the
    // South Africa subset. What follows is a mean-preserving reconstruction:
    // the rows are sorted on antibiotic prescription and the first 37 are
    // flagged, which reproduces the correct proportion for this subset.
    //
    // It is valid ONLY for the plotted mean. The assignment of the flag to
    // individual rows is not the real assignment, so any analysis that reads
    // this variable at the row level, or crosses it with another variable,
    // will be wrong. Do not use it for any other analysis.

    gsort - antibio
    gen med_any = (_n <= 37)
      label var med_any "Reconstructed: valid for the mean only, not row-level"
  ...
  // Other datasets
  use "$DATA_DIR/SP Interactions.dta" ///
...
// SP Characteristics & FE from Qutub Study
  ...
  // Source file: analysis_baseline.dta, the two-city SP baseline extract.
  // Place it in $DATA_DIR alongside the other granted files.

  use "$DATA_DIR/analysis_baseline.dta" , clear
